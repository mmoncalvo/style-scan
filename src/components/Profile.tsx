  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="col-span-1 bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden transition-colors duration-300">
          <div className="bg-[#0B5C66] dark:bg-teal-900/40 p-8 text-white">
            <div className="flex items-center gap-6">
              <div className="min-w-16 h-16 bg-white/20 dark:bg-slate-600/60 backdrop-blur-md rounded-2xl flex items-center justify-center text-3xl font-bold">
                {user.username.charAt(0).toUpperCase()}
              </div>
              <div className="flex-grow space-y-2">
                <h2 className="text-xl font-bold dark:text-white">{user.fullName || user.username}</h2>
                <p className="text-xs text-white/70 dark:text-teal-400/70 flex gap-2 text-nowrap">
          ...
          <div className="p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-300 ml-1">Cambiar Contraseña (dejar en blanco para mantener actual)</label>
                <div className="relative">
              ...
              <div className="flex items-center justify-between gap-4">
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full p-2 px-4 bg-[#0B5C66] dark:bg-teal-600 text-white rounded-2xl font-bold shadow-lg shadow-[#0B5C66]/20 dark:shadow-teal-900/20 hover:bg-[#0B5C66]/90 dark:hover:bg-teal-700 transition-all flex items-center justify-center gap-2"
                >
                  {isLoading ? (
                ...
                <button
                  onClick={onLogout}
                  className="p-2 px-4 flex items-center justify-center text-rose-500 border border-rose-700 rounded-2xl font-bold transition-all cursor-pointer gap-2"
                  title="Cerrar Sesión"
                >
