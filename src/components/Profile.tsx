  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-[#0B5C66] p-8 text-white">
          <div className="flex items-center gap-6">
            <div className="w-20 h-20 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center text-3xl font-bold">
              {user.username.charAt(0).toUpperCase()}
            </div>
            <div className="flex-grow">
              <h2 className="text-2xl font-bold">{user.fullName || user.username}</h2>
              <p className="text-white/70">@{user.username} • {user.role === 'admin' ? 'Administrador' : 'Cliente'}</p>
            </div>
            <button
              onClick={onLogout}
        ...
        <div className="p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700 ml-1">Cambiar Contraseña (dejar en blanco para mantener actual)</label>
              <div className="relative">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <HistoryIcon className="w-6 h-6 text-[#0B5C66]" />
            Mis Análisis
          </h3>
        </div>
