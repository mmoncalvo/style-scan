...
async function pollTask(taskId: string, apiKey: string, baseUrl: string) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      console.log(`[pollTask] Status: ${response.status} ${response.statusText}`);
      console.log(`[pollTask] Response Data:`, JSON.stringify(response.data, null, 2));

      const taskStatus = response.data?.data?.task_status;
      if (taskStatus === 'success') {
        console.log(`[pollTask] Task ${taskId} succeeded!`);
        return response.data?.data?.results;
      }
      if (taskStatus === 'error') {
        console.error(`[pollTask] Task ${taskId} reported error:`, JSON.stringify(response.data?.data?.error_msg || response.data?.data));
        throw new Error(`Task failed: ${JSON.stringify(response.data)}`);
      }
      console.log(`[pollTask] Task ${taskId} status: ${taskStatus}. Waiting...`);
    } catch (pollErr: any) {
      console.error(`[pollTask] Error during polling attempt ${attempt}:`, pollErr.response?.data || pollErr.message);
      if (attempt === maxAttempts) throw pollErr;
    }
...
async function startServer() {
  ...
  console.log(">>> [startServer] Env keys:", Object.keys(process.env).filter(k => k.includes('PERFECT')));
  ...
  app.post("/api/analyze", upload.single('image'), async (req: any, res) => {
    try {
      ...
      let baseUrl = process.env.PERFECT_CORP_API_URL || "https://yce-api-01.makeupar.com/s2s/v2.0/task/skin-analysis";
      
      // Auto-fix common URL truncation issues
      if (baseUrl.endsWith('/v2.0') || baseUrl.endsWith('/v2.0/')) {
        const originalUrl = baseUrl;
        baseUrl = baseUrl.replace(/\/$/, '') + '/task/skin-analysis';
        console.warn(`[analyze] API URL appeared truncated (${originalUrl}), adjusted to: ${baseUrl}`);
      }
      ...
      console.log(`[analyze] API URL: ${baseUrl}`);
      console.log(`[analyze] API Key (first 4 chars): ${apiKey ? apiKey.substring(0, 4) : 'MISSING'}...`);

      // 1. Read file and convert to Base64
      const imageBuffer = fs.readFileSync(req.file.path);
      const base64Image = imageBuffer.toString('base64');
      console.log(`[analyze] Image read, Base64 length: ${base64Image.length}`);
      ...
      console.log(`[analyze] Request Body Keys: ${Object.keys(requestBody).join(', ')}`);
      
      const startResponse = await axios.post(baseUrl, requestBody, {
        headers: {
      ...
      console.log(`[analyze] Start Response Status: ${startResponse.status} ${startResponse.statusText}`);
      console.log(`[analyze] Start Response Data:`, JSON.stringify(startResponse.data, null, 2));
      ...
      console.error(">>> [analyze] CRITICAL ERROR:");
      if (error.response) {
        console.error(`[analyze] Status: ${error.response.status}`);
        console.error(`[analyze] Data:`, JSON.stringify(error.response.data, null, 2));
        console.error(`[analyze] Headers:`, JSON.stringify(error.response.headers, null, 2));
      } else {
        console.error(`[analyze] Message: ${error.message}`);
      }
      res.status(500).json({ 
